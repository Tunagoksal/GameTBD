...
bool loadMedia()
{
	...
	//Open the font
	gFont = TTF_OpenFont( R"(D:\C++\projects\GameTBD\assets\Paul-le1V.ttf)", 28 );
	if( gFont == nullptr )
	{
	...
	else
	{
        ...
        if(!gTimePromtTexture.loadFromRenderedText("Seconds since start time",textColor)){
	...
	else
	{
		...
		else
		{
			...
			while( !quit )
			{
				...
				//Render textures
                gTimePromtTexture.render((SCREEN_WIDTH - gTimePromtTexture.getWidth() ) / 2, gStartPromptTexture.getHeight() + gPausePromptTexture.getHeight());
